...
cor.test(df_working$settle_size, df_sub$R_DEN, method = "spearman")
...
# positive impact of the size of the settlement on wages
...
# further more rho is about 60% so the relationship between educ level and level of occuption is high
...
# ie there is an impact of gender on occupation

tapply(df_sub$occ, df_sub$female, median, na.rm = TRUE)
#   0   1
#   4   6
#
# WARNING: occ was RECODED so that higher values = LOWER status occupations
#   1 = Director (highest status)
#   8 = Unqualified worker (lowest status)
#
# → Men   (0): median occ = 4 → "Employees engaged in documentation/accounting"
# → Women (1): median occ = 6 → "Qualified workers of agriculture and forestry"
#
# → Women have a higher median rank (6 > 4), meaning they are concentrated
#   in lower-status occupations than men on average.
# → This is consistent with the Mann-Whitney result (p < 2.2e-16).
...
# of the "alternative hypothesis" : mean wages are different for men and women
...
# Furthermore inspecting the mean for group 0 and 1 shows that mean wage for mean
# are greated than mean wages for women (about 30% difference)
...
m1 <- lm(R_DEN ~ female + H01_02 + edu4_scale + settle_size + occ, data = df_sub)
summary(m1)
# Coefficients:
...
m3 <- lm(R_DEN ~ female + H01_02 + edu4_scale + settle_size + occ + large_family, data = df_sub)
summary(m3)
# Coefficients:
#                 Estimate Std. Error t value Pr(>|t|)    
